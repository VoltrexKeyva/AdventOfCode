...
int main() {
  ...
  size_t chars_processed = START_OF_MESSAGE_LEN;
  ...
  for (int i = 0; i < datastream_buf.length(); i++, chars_processed++) {
    const string charset = datastream_buf.substr(i, START_OF_MESSAGE_LEN);
    const set<char> unique_charset(charset.begin(), charset.end());

    if (unique_charset.size() == START_OF_MESSAGE_LEN) break;
  }
